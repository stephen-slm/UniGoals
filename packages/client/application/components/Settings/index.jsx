...
class Settings extends React.Component {
  ...
  handleFormChange = (type, change) => {
    ...
    profile.settings[type] = change.target.value;
  ...
  handleColorChange = (type, change) => {
    const content = change;
    content.target.value = Settings.colourNameToHex(change.target.value);

    this.handleFormChange(type, content);
  };
  ...
  handleFormSave = () => {
    ...
    const updatedProfile = Object.assign(profile, {
      settings: {
        primarycolor: this.state.profile.settings.primarycolor,
        secondarycolor: this.state.profile.settings.secondarycolor,
      },
    });
  ...
  resetDefaults = () => {
    const primaryChange = { target: { value: '#3A506B' } };
    const secondChange = { target: { value: '#5BC0BE' } };

    this.handleFormChange('primarycolor', primaryChange);
    this.handleFormChange('secondarycolor', secondChange);
  };
        ...
        <Paper square elevation={1} className={classes.paper}>
          <form className={classes.formContainer} noValidate autoComplete="off">
            <div>
              ...
              />
              <span style={{ color: this.state.profile.settings.primarycolor }}>Preview</span>
            </div>
            <div>
              <TextField
                margin="normal"
                ...
                label="Secondary Color"
                className={classes.textField}
                value={this.state.profile.settings.secondarycolor}
                onChange={(change) => this.handleColorChange('secondarycolor', change)}
              />
